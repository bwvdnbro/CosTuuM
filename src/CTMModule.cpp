...
/**
 * @brief Get a table of scattering matrices for the given input dust grain
 * sizes, dust grain types, wavelengths and angles, using the given shape and
 * alignment distributions.
 *
 * Required arguments are:
 ...
 *  - sizes: Array of dust grain sizes (in m).
 *  - wavelengths: Array of incoming/outgoing photon wavelengths (in m).
 *  - theta_in: Array of incoming zenith angles (in radians).
 *  - theta_out: Array of outgoing zenith angles (in radians).
 *  - phi: Array of relative azimuth angles (in radians).
 *  - shape_distribution: Shape distribution object to use.
 *  - alignment_distribution: Alignment distribution object to use.
 *  - dust_properties: Dust properties object to use.
 *
 ...
 * @return Nothing.
 */
static PyObject *get_scattering_matrix_table(PyObject *self, PyObject *args,
                                             PyObject *kwargs) {
  ...
  PyArrayObject *input_sizes;
  PyArrayObject *input_wavelengths;
  PyArrayObject *input_theta_in;
  PyArrayObject *input_theta_out;
  PyArrayObject *input_phi;
  PyShapeDistribution *shape_distribution_object;
  PyAlignmentDistribution *alignment_distribution_object;
  PyDustProperties *dust_properties_object;
  ...
  if (!PyArg_ParseTupleAndKeywords(
          ...
          &input_memory_log_name, &verbose_b)) {
    // again, we do not call ctm_error to avoid killing the Python interpreter
    ctm_warning("Wrong arguments provided!");
    // this time, a nullptr return will signal an error to Python
    return nullptr;
  ...
  std::vector<float_type> theta_in =
      unpack_numpy_array<float_type, double>(input_theta_in);
  Py_DECREF(input_theta_in);
  std::vector<float_type> theta_out =
      unpack_numpy_array<float_type, double>(input_theta_out);
  Py_DECREF(input_theta_out);
  std::vector<float_type> phi =
      unpack_numpy_array<float_type, double>(input_phi);
  Py_DECREF(input_phi);
  ...
  std::vector<Result *> results;
  TMatrixAuxiliarySpaceManager *space_manager = nullptr;
  task_manager.generate_scattering_tasks(
      theta_in, theta_out, phi, quicksched, tasks, resources, result_key,
      results, space_manager, verbose, input_memory_log_name != nullptr,
      input_memory_log);
  ...
  npy_intp result_dims[8] = {
      static_cast<npy_intp>(result_key->composition_size()),
      static_cast<npy_intp>(result_key->size_size()),
      static_cast<npy_intp>(result_key->wavelength_size()),
      static_cast<npy_intp>(theta_in.size()),
      static_cast<npy_intp>(theta_out.size()),
      static_cast<npy_intp>(phi.size()),
      4,
      4};
  PyArrayObject *result_array = reinterpret_cast<PyArrayObject *>(
      PyArray_SimpleNew(8, result_dims, NPY_DOUBLE));
  ...
  for (npy_intp itype = 0; itype < result_dims[0]; ++itype) {
    for (npy_intp isize = 0; isize < result_dims[1]; ++isize) {
      for (npy_intp ilambda = 0; ilambda < result_dims[2]; ++ilambda) {
        ...
        for (npy_intp itheta_in = 0; itheta_in < result_dims[3]; ++itheta_in) {
          for (npy_intp itheta_out = 0; itheta_out < result_dims[4];
               ++itheta_out) {
            for (npy_intp iphi = 0; iphi < result_dims[5]; ++iphi) {
              float_type Z[16];
              result.get_scattering_matrix(itheta_in, itheta_out, iphi, Z);
              for (npy_intp irow = 0; irow < 4; ++irow) {
                for (npy_intp icol = 0; icol < 4; ++icol) {
                  npy_intp array_index[8] = {itype,     isize,      ilambda,
                                             itheta_in, itheta_out, iphi,
                                             irow,      icol};
                  *reinterpret_cast<double *>(
                      PyArray_GetPtr(result_array, array_index)) =
                      static_cast<double>(Z[4 * irow + icol]);
                } // col
              }   // row
            }     // phi
          }       // theta_out
  ...
  PyObject *return_value = PyArray_Squeeze(result_array);
...
static PyMethodDef CTMmethods[] = {
    ...
    {"get_table", reinterpret_cast<PyCFunction>(get_table),
     ...
     "module."},
    {"get_scattering_matrix_table",
     reinterpret_cast<PyCFunction>(get_scattering_matrix_table),
     METH_VARARGS | METH_KEYWORDS,
     "Function that computes a table of scattering matrices."},
